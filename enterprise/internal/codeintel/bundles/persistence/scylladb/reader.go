...
func (r *reader) ReadMeta(ctx context.Context) (types.MetaData, error) {
	...
	if err := session.Query(
		...
		r.dumpID,
	).Consistency(gocql.One).Scan(&numResultChunks); err != nil {
...
func (r *reader) ReadDocument(ctx context.Context, path string) (types.DocumentData, bool, error) {
	...
	if err := session.Query(
		...
		r.dumpID,
		path,
	).Consistency(gocql.One).Scan(&data); err != nil {
...
func (r *reader) defref(ctx context.Context, tableName, identifier, scheme string, skip, take int) ([]types.Location, int, error) {
